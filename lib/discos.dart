...
import 'adddiscos.dart';

class DiscosPage extends StatefulWidget {
  const DiscosPage({super.key});
...
class _DiscosPageState extends State<DiscosPage> {
  ...
  Future loadData() async {
    final data = await DatabaseHelper.instance.getAll("discos");
    setState(() => itens = data);
  }

  Future deleteItem(int id) async {
    await DatabaseHelper.instance.delete("discos", id);
    loadData();
  }
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () async {
              await DatabaseHelper.instance.update("discos", {
                "id": item["id"],
                "nome": nome.text,
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          await Navigator.push(
              context,
              MaterialPageRoute(builder: (_) => const AddDiscos()));
          loadData();
        },
      ),
      body: Column(
